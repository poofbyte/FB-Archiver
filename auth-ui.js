(function(){const u=globalThis.Auth,i=globalThis.SiteConfig||{},A=i.SITE_NAME||"Downloader",b=i.HELP_URL||i.helpUrl||"https://help.serp.co/en/",w=i.helpLinkLabel||"Need help?";try{const t=document.getElementById("bootSplash");t&&setTimeout(()=>{try{t.classList.add("hidden")}catch{}},2500)}catch{}const C=`
body[data-auth-screen="visible"] .header {
  display: flex;
...
body[data-auth-screen="visible"] .header h1,
body[data-auth-screen="visible"] .header h2 {
  font-size: 15px;
  font-weight: 600;
...
body[data-auth-screen="visible"] .header > :first-child {
  flex: 1 1 auto;
  min-width: 0;
}
...
body[data-auth-screen="visible"] .top-help-link {
  margin-left: auto;
  margin-top: 0;
...
`;function l(t,a,e){if(t&&(t.textContent=a||"",t.className="activation-status",e&&(t.classList.add(e),t.classList.add("status-"+e)),t.classList.remove("hidden"),e!=="success")){try{clearTimeout(t.__hideTimer)}catch{}t.__hideTimer=setTimeout(()=>{try{t.classList.add("hidden")}catch{}},5e3)}}function B(){if(document.getElementById("serp-auth-ui-styles"))return;const t=document.createElement("style");t.id="serp-auth-ui-styles",t.textContent=C,(document.head||document.documentElement).appendChild(t)}function I(t,a){const e=i.activationHeading||"Get started in seconds",f=i.activationSubtitle||"Enter your email to activate the extension. We'll send a quick verification code - no password needed.",v=i.sendCodeLabel||"Send verification code",s=i.verifyCodeLabel||"Verify & Continue",p=i.upgradePrefixText||"Want unlimited downloads?",g=i.buyLinkLabel||"Get a license ->",h=i.trialPillText||"3 free downloads included";t.innerHTML=`
      ...
      <div class="activation-panel">
        ...
        <div class="activation-actions">
          <button id="sendCodeBtn" class="activate-btn" type="button">${v}</button>
